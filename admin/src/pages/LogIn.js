            ...
            <form onSubmit = { submitHandler } >
                <div className = { `${style.loginMainContainer}` } >
                    ...
                    <div className = { `${style.inputContainer}` } >
                        <input type = "text" name = "email" id = "login-email-input" className = "login-input-box" placeholder = "Username..." required onChange = { e => setDetails({...details, email: e.target.value }) } value = { details.email }/>
                    </div>
                    ...
                    { /* error message  */ } {
                    (error != "") ? ( < div className = { `${style.errorDisplay}` } > { error } </div>):""}
